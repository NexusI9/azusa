...
public class FreeFlyCamera : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("Camera orbit around origin on click")]
    ...
    private void Update()
    {
        ...
        if (_enableMovement)
        {
            ...
            if (Input.GetKey(KeyCode.W))
                deltaPosition += transform.InverseTransformDirection(transform.forward);
        ...
        if(_enableOrbit)
        {
            ...
            {
                transform.RotateAround(Vector3.zero, Vector3.up, Input.GetAxis("Mouse X") * _mouseSense);
            }
        }
